...
func TestSealedStoreEncryptDecrypt(t *testing.T) {
	s := MustNewStore("test-password", "0123456789abcdef0123456789abcdef")
...
func TestSealedStoreUpdatePreservesEncryption(t *testing.T) {
	s := MustNewStore("pass", "0123456789abcdef0123456789abcdef")
	s.Set("key", "v1")
	s.Set("key", "v2")
...
func TestHandleHealth(t *testing.T) {
	srv, _ := newTestServerForMain(MustNewStore("", ""))
...
func TestHandleSecretsPostInvalidName(t *testing.T) {
	srv, _ := newTestServerForMain(MustNewStore("", ""))

	body := `{"name":"bad;name` + "`" + `","value":"val"}`
	req := httptest.NewRequest("POST", "/secrets", bytes.NewBufferString(body))
	req.Header.Set("X-Vault-Token", "test-admin-token")
	w := httptest.NewRecorder()
	srv.handleSecrets(w, req)
	if w.Code != http.StatusBadRequest {
		t.Fatalf("expected 400 for invalid secret name, got %d", w.Code)
	}
	if !strings.Contains(w.Body.String(), "invalid secret name") {
		t.Fatalf("expected invalid secret name message, got: %s", w.Body.String())
	}
}
...
func TestHandleAccessSecretNotFound(t *testing.T) {
	srv, cfg := newTestServerForMain(MustNewStore("", ""))
	token := "tok1"
	cfg.SecretTokens[hashToken(token)] = &SecretToken{
...
func TestServerDevMode(t *testing.T) {
	...
	srv := NewServer(MustNewStore("", ""), cfg, "")
	...
	srvProd := NewServer(MustNewStore("", ""), cfgProd, "")
	...
	srvProdFallback := NewServer(MustNewStore("", ""), cfgProdFallback, "")
...
// Extra Handlers testing
func TestServer_ExtraHandlers(t *testing.T) {
	store := MustNewStore("password", "73616c74313233343536373839303132")
	...
	// Project by ID Handler
	req2, _ := http.NewRequest("GET", "/projects/proj1", nil)
	req2.Header.Set("X-Vault-Token", "admin")
	rr2 := httptest.NewRecorder()
	srv.handleProjectByID(rr2, req2)
	
	// Project Tokens Handler
	req3, _ := http.NewRequest("GET", "/projects/proj1/tokens", nil)
	req3.Header.Set("X-Vault-Token", "admin")
	rr3 := httptest.NewRecorder()
	srv.handleProjectTokens(rr3, req3)
	
	// Audit Handler
	req4, _ := http.NewRequest("GET", "/audit", nil)
	req4.Header.Set("X-Vault-Token", "admin")
	rr4 := httptest.NewRecorder()
	srv.handleAudit(rr4, req4)
	
	// Token Handler
	req5, _ := http.NewRequest("GET", "/token/123", nil)
	req5.Header.Set("X-Vault-Token", "admin")
	rr5 := httptest.NewRecorder()
	srv.handleToken(rr5, req5)
	
	// Secret By Name
	req6, _ := http.NewRequest("GET", "/secret/sec1", nil)
	req6.Header.Set("X-Vault-Token", "admin")
	rr6 := httptest.NewRecorder()
	srv.handleSecretByName(rr6, req6)
	
	// Secret Get
	req7, _ := http.NewRequest("GET", "/secret/sec1", nil)
	rr7 := httptest.NewRecorder()
	srv.handleSecretGet(rr7, req7, "sec1")
	
	// Config save wrapper (already tested, just calling to hit branches)
