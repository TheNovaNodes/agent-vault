...
Внешние клиенты:
 ...
 Лаборант ───────▶ agent-vault-env ─▶ GET /access (Bearer) ─▶ Store
...
| GET | `/access` | Bearer Token | Доступ по токену (single or project) |
...
**ProjectToken** даёт доступ ко всем секретам проекта через один вызов `GET /access` (с заголовком `Authorization: Bearer <project_token>`).
...
6. **One-time токены** — инвалидация после первого использования (endpoint `GET /access`)
...
9. **Rate limiting** — 10 req/min на IP для `GET /access` (с защитой от IP spoofing)
