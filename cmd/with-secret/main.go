...
var (
	pointerIDRegex = regexp.MustCompile(`^[a-zA-Z0-9_\-]+$`)
...
func run() int {
	...
	if filepath.Base(pointerID) != pointerID || pointerID == "." || pointerID == ".." || !pointerIDRegex.MatchString(pointerID) {
		fmt.Fprintf(os.Stderr, "❌ Error: Invalid secret pointer ID %q.\n", pointerID)
		return 1
	}

	if !varNameRegex.MatchString(varName) {
		fmt.Fprintf(os.Stderr, "❌ Error: Invalid environment variable name %q.\n", varName)
		return 1
	}
