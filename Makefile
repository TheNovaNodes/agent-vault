...
WITH_SECRET_BINARY=with-secret
GO=go
GOFLAGS=-v
CONFIG=config.yaml
...
build: $(BINARY) $(ENV_BINARY) $(CLI_BINARY) $(WITH_SECRET_BINARY) ## Собрать все бинарники
...
$(WITH_SECRET_BINARY): cmd/with-secret/*.go
	$(GO) build $(GOFLAGS) -o $(WITH_SECRET_BINARY) ./cmd/with-secret
...
clean: ## Очистка артефактов сборки
	rm -f $(BINARY) $(ENV_BINARY) $(CLI_BINARY) $(WITH_SECRET_BINARY)
